...
BUILD_TAG = "Smart UOA Classifier: Buy vs Avoid Labels with Dynamic Targets"
...
with col_title:
  ...
  st.caption("內置 UOA 智能過濾分類器：自動標記【建議買入】與【嚴禁買入（彩票陷阱）】")
with col_ver:
  st.markdown(
        ...
        for _, c_row in calls_df.iterrows():
          ...
          if c_vol >= min_vol and c_ratio >= min_ratio:
            ...
            if otm_distance_pct > 0.12:
              ...
              rec_badge = "🟢 建議買入"
              reason = (
                  f"輕度虛值 (+{round(otm_distance_pct*100, 1)}%)，知情大單爆量掃盤，小資金高勝率"
              )
              tp_target = (
...
st.markdown(
    ...
    f" ({BUILD_DATE}) · Built-in UOA Classifier</div>",
    unsafe_allow_html=True,
)
