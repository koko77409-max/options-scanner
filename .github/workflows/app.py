...
BUILD_TAG = "Auto-Cruise Patrol Enabled: Periodic Background Scanner"
...
SECTOR_MAP = {
    ...
    "AMZN": "電商/雲端", "META": "社交/廣告", "GOOGL": "搜尋/雲端", "AMD": "半導體/AI",
    "AVGO": "半導體/AI", "TSM": "半導體/AI", "QCOM": "半導體/AI", "ASML": "半導體/AI",
    "MU": "半導體/AI", "ARM": "半導體/AI", "INTC": "半導體/AI", "SMCI": "半導體/AI",
    "PLTR": "大數據/AI", "CRWD": "網絡安全", "PANW": "網絡安全", "SNOW": "數據倉庫",
    "NET": "雲端網絡", "DDOG": "雲監控", "CRM": "企業軟件", "ORCL": "企業軟件",
    "SHOP": "電子商務", "PATH": "企業軟件", "COIN": "加密貨幣", "MSTR": "加密貨幣",
    "MARA": "加密貨幣", "HOOD": "金融科技", "AFRM": "金融科技", "SOFI": "金融科技",
    "UPST": "金融科技", "UBER": "移動出行", "DIS": "娛樂傳媒", "NFLX": "串流媒體",
    "RBLX": "元宇宙/遊戲", "DKNG": "在線博彩", "COST": "消費零售", "WMT": "消費零售",
    "BABA": "中概互聯", "PDD": "中概互聯", "JD": "中概互聯", "NIO": "中概互聯",
    "RIVN": "新能源車", "BA": "航空航天", "JPM": "傳統金融", "GS": "傳統金融",
    "BAC": "傳統金融", "XOM": "傳統能源", "CVX": "傳統能源", "OXY": "傳統能源",
...
def scan_pure_flow(tickers, d_min, d_max, min_uoa_v, min_uoa_r, budget, min_rr, check_liq, avoid_earn, min_notional, min_gain_pct):
                ...
                for _, c_row in calls_df.iterrows():
                    ...
                    if c_vol >= min_uoa_v and c_ratio >= min_uoa_r:
                        ...
                        if c_pct_change < min_gain_pct:
                            ...
                            rec_badge = "🟢 建議買入"
                            reason = f"主動追價掃盤 (漲幅 +{round(c_pct_change, 1)}%)，總額 ${int(notional_usd):,}，主力動能極強"
                            ...
                            if not s_cands.empty:
                                ...
                                b_p = c_price
                                s_p = float(s_leg.get("bid", 0)) if float(s_leg.get("bid", 0)) > 0 else float(s_leg.get("lastPrice", 0))
                                ...
                                if rr >= min_rr and cost <= budget:
                                    spread_recommendations.append({
                                        ...
                                        "開倉限價 (單價)": f"${net_debit}",
                                        "單手成本 ($)": f"${cost}",
                                        "目標止盈限價": f"${round(net_debit + (max_profit*0.5/100), 2)} ~ ${round(net_debit + (max_profit*0.7/100), 2)}",
                        ...
                        uoa_alerts.append({
                            ...
                            "標的代號": sym,
                            "方向類型": "🟢 看漲 (Call 異動)",
                            "合約當日漲跌": f"{'+' if c_pct_change>0 else ''}{round(c_pct_change, 1)}%",
                            "到期日": f"{target_exp} ({target_dte}天)",
                ...
                for _, p_row in puts_df.iterrows():
                    ...
                    if p_vol >= min_uoa_v and p_ratio >= min_uoa_r:
                        ...
                        if p_pct_change < min_gain_pct:
                            ...
                            rec_badge = "🟢 建議買入"
                            reason = f"主動追價做空 (漲幅 +{round(p_pct_change, 1)}%)，總額 ${int(notional_usd):,}，主力爆量押注大跌"
                        ...
                        uoa_alerts.append({
                            ...
                            "標的代號": sym,
                            "方向類型": "🔴 做空 (Put 異動)",
                            "合約當日漲跌": f"{'+' if p_pct_change>0 else ''}{round(p_pct_change, 1)}%",
                            "到期日": f"{target_exp} ({target_dte}天)",
    ...
    if not df_uoa.empty:
        df_uoa["sort_key"] = df_uoa["推薦評級"].apply(lambda x: 0 if "建議買入" in x else 1)
...
if "pure_uoa_df" in st.session_state and "pure_spread_df" in st.session_state:
    ...
    rec_count = len(uoa_df[uoa_df["推薦評級"].str.contains("建議買入")]) if not uoa_df.empty else 0
...
if auto_scan_enabled:
    ...
    for remaining in range(scan_interval_min * 60, 0, -1):
        ...
        countdown_container.info(f"⏱️ **定時巡航運行中**：下次自動重新掃描倒數 **{mins:02d}分 {secs:02d}秒**...（可隨時取消側邊欄勾選）")
        time.sleep(1)
    run_scan()
    st.rerun()
