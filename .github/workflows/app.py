...
BUILD_TAG = "Expanded Golden Watchlist (70 High-Liquidity Power Assets)"
...
with col_title:
    ...
    st.caption("黃金 70 隻高流動性標的池 + TTM Squeeze 埋伏 + UOA 異動買入/避坑智能分類")
with col_ver:
    st.markdown(
...
DEFAULT_WATCHLIST = [
    ...
    # 半導體與硬件鏈
    "AMD", "AVGO", "TSM", "QCOM", "ASML", "MU", "ARM", "INTC", "SMCI",
    # 網絡安全、AI 與軟件 SaaS
    "PLTR", "CRWD", "PANW", "SNOW", "NET", "DDOG", "CRM", "ORCL", "SHOP", "PATH",
    # 加密貨幣與金融科技
    "COIN", "MSTR", "MARA", "HOOD", "AFRM", "SOFI", "UPST",
    # 成長與消費零售
    "UBER", "DIS", "NFLX", "RBLX", "DKNG", "COST", "WMT",
    # 中概熱門
    "BABA", "PDD", "JD", "NIO",
    # 新能源與車企
    "RIVN", "BA",
...
def get_options_spreads_and_uoa(candidates_df, d_min, d_max, tp_min_pct, tp_max_pct, sl_pct, min_rr, check_liq, min_uoa_v, min_uoa_r, budget):
    spreads_columns = [
        "標的代號", "方向", "正股現價", "到期日", "策略", "【買入行使價】",
        "【賣出行使價】", "開倉限價 (單價)", "單手成本 ($)", "目標止盈限價", "剛性止損價位", "盈虧比", "Cost_Num", "RR_Num"
    ]
    uoa_columns = [
        "推薦評級", "標的代號", "到期日", "異動行使價", "單張成本 ($)",
        "建議買入限價", "止盈目標價 (+60%)", "止損底線 (-35%)", "成交量 / OI (倍數)", "系統判定理由"
    ]
            ...
            if not calls_df.empty:
                for _, c_row in calls_df.iterrows():
                    ...
                    if c_vol >= min_uoa_v and c_ratio >= min_uoa_r:
                        ...
                        otm_distance_pct = (c_strike - curr_price) / curr_price if curr_price > 0 else 0.0

                        if otm_distance_pct > 0.12:
                            rec_badge = "🔴 嚴禁買入"
                            reason = f"深度虛值 OTM (+{round(otm_distance_pct*100, 1)}%)，彩票陷阱 / 機構賣出腿"
                            tp_target = "不適用"
                            sl_target = "不適用"
                            ...
                            rec_badge = "🟢 建議買入"
                            reason = f"輕度虛值 (+{round(otm_distance_pct*100, 1)}%)，知情大單爆量掃盤，小資金高勝率"
...
if st.button("🚀 開始全市場量化埋伏與主力異動掃描"):
    with st.spinner("正在對 70 隻核心資產執行 K 線壓縮與期權鏈全量穿透分析..."):
        ...
        spread_df, uoa_df = get_options_spreads_and_uoa(
            cand_df, dte_min, dte_max, tp_ratio_min, tp_ratio_max, sl_ratio, min_rr_ratio, filter_liquidity, min_uoa_vol, min_vol_oi_ratio, max_budget
        )
...
if "cand_df" in st.session_state and "spread_df" in st.session_state and "uoa_df" in st.session_state:
    ...
    col4.metric("🚨 異動監控", f"{len(uoa_df)} 個合約")

    # 1. 垂直價差組合
    st.markdown(f"### 🎯 精選垂直價差指令板 (小資金穩健對沖)")
    ...
    # 2. UOA 主力異動雷達
    st.markdown("### 🚨 期權異常異動雷達 (智能買入 / 避坑分類)")
    if not uoa_df.empty:
...
st.markdown(
    ...
    f"OptionsQuant Pro Engine · Release {APP_VERSION} ({BUILD_DATE}) · 70 Golden Assets Pool"
    f"</div>",
    unsafe_allow_html=True,
)
