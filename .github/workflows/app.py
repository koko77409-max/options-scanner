...
BUILD_TAG = "Auto-Cruise Patrol & Multi-Expiration Scanner Enabled"
...
scan_interval_min = st.sidebar.slider("掃描間隔 (分鐘)", 3, 15, 5, 1)
...
filter_liquidity = st.sidebar.checkbox(
    "自動過濾寬點差合約 (點差 < $0.40)", value=True
)
...
with col_title:
  ...
  st.caption("內置定時無人巡航 + 多到期日覆蓋 + 自動寫入交易日誌")
with col_ver:
  st.markdown(
...
def scan_pure_flow(
  ...
  for sym in tickers:
    try:
      ...
      # 覆蓋範圍內符合條件的所有主要到期日 (最多檢查 2 個，避免頻繁請求被 Yahoo 429)
      valid_expirations = []
      ...
      for target_exp, target_dte in valid_expirations:
...
# ==========================================
def run_scan_cycle():
  with st.spinner("🔍 正在穿透監控標的多期權鏈，執行動能與資金流算法..."):
    ...
    if not uoa_df.empty:
      ...
      if not rec_buys.empty:
        ...
        st.toast(
            f"✅ 成功將 {len(rec_buys)} 個「建議買入」合約寫入歷史紀錄本！",
            icon="📝",
        )


btn_clicked = st.button("🚀 手動觸發全市場主力資金流 (UOA) 穿透掃描")
if btn_clicked:
  run_scan_cycle()

# 如果開啟定時巡航且沒有正在顯示的倒計時，啟動定時輪詢
...
st.markdown(
    ...
    f" ({BUILD_DATE}) · Auto-Cruise Enabled</div>",
    unsafe_allow_html=True,
)
