...
st.caption(
    "基於 TTM Squeeze 波動率收斂 + 均線多空排列 + 跨價期權組合 (Vertical"
    " Spreads) 自動止盈止損定價"
)
...
def run_scan(tickers, atr_mult):
  ...
  for sym in tickers:
    try:
      ...
      vol_ratio = (
          round(float(vol.iloc[-1]) / vol_ma20, 2) if vol_ma20 > 0 else 1.0
      )
...
def get_options_spreads(
    candidates_df, d_min, d_max, tp_min_pct, tp_max_pct, sl_pct
):
    ...
    try:
      ...
      if "CALL" in direction:
        ...
        if b_cands.empty or s_cands.empty:
          continue
        b_leg, s_leg = b_cands.iloc[0], s_cands.iloc[0]
        b_p = b_leg["ask"] if b_leg["ask"] > 0 else b_leg["lastPrice"]
        s_p = s_leg["bid"] if s_leg["bid"] > 0 else s_leg["lastPrice"]
      ...
      # ---------------- 空頭 (Bear Put Spread) ----------------
        ...
        if b_cands.empty or s_cands.empty:
          continue
        b_leg, s_leg = b_cands.iloc[-1], s_cands.iloc[-1]
        b_p = b_leg["ask"] if b_leg["ask"] > 0 else b_leg["lastPrice"]
        s_p = s_leg["bid"] if s_leg["bid"] > 0 else s_leg["lastPrice"]
